(function() {
    ...
    function assignCss(dest) {
        ...
        for (var i = 1; i < len; i++) {
            var src = arguments[i];
            if (!src) continue;
            var srcKeys = Object.keys(src), srcKeyLen = srcKeys.length;
    ...
    function addNewSelector(holder, selector, delimiter, subSelector, value, uniq) {
        ...
        if (typeof value === 'string') add(holder, newSelector, value); else {
    ...
    function o2css(selector, styles, uniq) {
        var flattenStyles = {
            __proto__: null
        }, keys = Object.keys(styles), len = keys.length, keyMap = {};
        for (var i = 0; i < len; i++) {
            var rule = keys[i], value = styles[rule], ruleChar0 = rule.charAt(0), cssRv = '', subSel, subStyle, delimiter;
            if (ruleChar0 === '&') {
    ...
    function translate(styles, uniq) {
        var sKeys = Object.keys(styles), keyLen = sKeys.length;
        result = {
            __proto__: null,
            cssstr: '',
            styles: {}
        ...
        for (var i = 0; i < keyLen; i++) {
            var selector = sKeys[i], selectorVal = styles[selector];
            if (typeof selectorVal == 'string') result.cssstr += selectorVal; else {
    ...
    function getStyle(styles) {
        ...
        for (var i = 0; i < sKeys.length; i++) {
            ...
            if (typeof selectorVal === 'string') styleString += selector + '{' + selectorVal.trim() + '}'; else {
                var flattenStyles = o2css(selector, selectorVal);
                var flatKeys = Object.keys(flattenStyles), flatKeyLen = flatKeys.length;
                for (var j = 0; j < flatKeyLen; j++) {
                    var sel = flatKeys[j], selStyle = flattenStyles[sel];
    ...
    function h2r(hexa) {
        if (hexa.charAt(0) === '#') hexa = hexa.substr(1);
        var len = hexa.length;
        if (len == 3) return 'rgb(' + h62rgb(dupstr(hexa)) + ')';
        if (len == 4) return 'rgba(' + h82rgba(dupstr(hexa)) + ')';
        if (len == 6) return 'rgb(' + h62rgb(hexa) + ')';
        if (len == 8) return 'rgba(' + h82rgba(hexa) + ')';
        return 'rgb(0, 0, 0)';
    }
    function defineCss(dest, src) {
        var sels = Object.keys(src), len = sels.length;
        for (var i = 0; i < len; i++) {
            var selector = sels[i];
    ...
    function forEach(object, fn, thisArg) {
        var keys = Object.keys(object), len = keys.length;
        for (var i = 0; i < len; i++) {
            var key = keys[i];
    ...
    var LazyCss = function(defs, uniq) {
        ...
        depProps(holder, {
            ...
            append: {
                value: function() {
                    if (this.appended) this.holder.textContent = this.css; else this.holder = append(this.css);
                }
            }
        });
        return holder;
    };
    if (typeof define === 'function' && define.amd) define(LazyCss); else if (typeof module !== 'undefined' && module.exports) module.exports = LazyCss; else window.lazycss = LazyCss;
})();